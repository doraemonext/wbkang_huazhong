    ...
    for row in range(3, nrows):
        ...
        if client_type == "range":
            ...
            for cur in range(0, nrows):
                try:
                    now_true_no = int(sheet.cell_value(cur, 0))
                except ValueError:
                    continue
                if now_true_no > to_no:
                    break
                if from_no <= now_true_no <= to_no:
                    try:
                        client_identifier = str(int(get_actual_value(sheet, merged_cells, cur, 3)))
                    except ValueError:
                        continue
                    if not client_identifier:
                        continue
                    client_target = sheet.cell_value(cur, 5)
                    client_group = get_actual_value(sheet, merged_cells, cur, 2)
